    ...
    def f_create_time_domain_from_indices(self):
        ...
        if len(self.idx)==10 and self.idx[:3] in ['rmm','bs1','bs2']:
            ...
            if counter=='001':
                ...
                # As '005', but for 30 Aug 2002 to 4 Oct 2020
                # For Colin Goldblatt's AIRS analysis
                # Note Colin's data goes to 6 Mar 2021, but RMM indices only available to 4 Oct 2020
                header1+='Index amplitude >=1, time range 30 Aug 2002 to 4 Oct 2020  \n'
                time1=cftime.DatetimeGregorian(2002,8,30)
...
class DataConverter(object):
    ...
    def read_cube(self):
        ...
        elif self.source in ['cmap_sfc_5d']:
            self.filein1=os.path.join(self.basedir,self.source,'raw','precip.pentad.mean.nc')
        ...
        elif self.source in ['imergplp_sfc_30m','imergmcw_sfc_30m','imergv07amcw_sfc_30m','imergmts_sfc_30m','imergmt2_sfc_30m','imergnpl_sfc_30m','imergnp2_sfc_30m','imergtrm_sfc_30m','imergv07atrm_sfc_30m']:
        ...
        elif self.source in ['imergv07bmcw_sfc_30m','imergv07btrm_sfc_30m']:
            self.filein1=os.path.join(self.basedir,self.source,'raw',str(self.year),str(self.month).zfill(2),'3B-HHR.MS.MRG.3IMERG.'+str(self.year)+str(self.month).zfill(2)+'*V07B.nc')
        ...
        if self.source in ['sstrey_sfc_7d','imergplp_sfc_30m','imergmcw_sfc_30m','imergv07amcw_sfc_30m','imergv07bmcw_sfc_30m','imergmts_sfc_30m','imergmt2_sfc_30m','imergnpl_sfc_30m','imergnp2_sfc_30m','imergtrm_sfc_30m','imergv07atrm_sfc_30m','imergv07btrm_sfc_30m','trmm3b42v7_sfc_3h','ncepdoegg_zlev_d','ostial4nrttrp_sfc_d','ostial4reptrp_sfc_d']:
...
class ModifySource(object):
    ...
    def f_time_average(self,method=3):
        ...
        time1,time2=block_times(self,verbose=self.verbose)
        if (fnmatch.fnmatch(self.source1,'imerg???_sfc_30m') and fnmatch.fnmatch(self.source2,'imerg???_sfc_3h')) or (fnmatch.fnmatch(self.source1,'imergv07a???_sfc_30m') and fnmatch.fnmatch(self.source2,'imergv07a???_sfc_3h')) or (fnmatch.fnmatch(self.source1,'imergv07b???_sfc_30m') and fnmatch.fnmatch(self.source2,'imergv07b???_sfc_3h')):
        ...
        if self.frequency in ['d','3h']:
            ...
            if method==1:
                ...
                #
                # For this special case source1 must be
                # 'imergXXX_sfc_30m' (or 'imergv07aXXX_sfc_30m') and
                # source2 must be 'imergXXX_sfc_3h' (or
                # 'imergv07aXXX_sfc_3h')
                #
                # NB Time stamp convention for source2 is time stamp at
    ...
    def f_interpolate_time(self):
        ...
        if self.frequency=='d':
            ...
            if self.verbose==2:
                print(ss.format(self))
            time_constraint=set_time_constraint(self.time1_in,self.time2_in,calendar=self.calendar,verbose=self.verbose)
            x1=self.data_in.extract(time_constraint)
            self.cube_in=x1.concatenate_cube()
            # Interpolate in time
...
class CombineLatitudes(object):
    ...
    def __init__(self,**descriptor):
        ...
        if self.source1==self.source2:
            raise UserWarning('source1 and source2 are identical.')
        source_info(self)
...
class AnnualCycle(object):
    ...
    def f_subtract_anncycle(self):
        ...
        while yearc<=year_end:
            ...
            data_anom=create_cube(conv_float32(data_anom.data),data_anom)
